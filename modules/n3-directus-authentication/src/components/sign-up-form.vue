<template>
  <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md" v-if="rules">

    <div class="flex flex-row gap-3">
      <div class="grow">
        <div class="font-sans font-medium text-lg">
          {{ core_labels.first_name_label }}
        </div>
      ...
      <div class="grow">
        ...
        <q-input
          ...
          counter
          lazy-rules
          :rules="[rules(last_name, 'required'), rules(last_name, 'not_empty')]"
        />
      </div>
    </div>
    ...
    <div class="grow">
      ...
      <q-input
        ...
        counter
        lazy-rules
        :rules="[rules(email, 'required'), rules(email, 'is_email_format')]"
      />
    </div>

    <div class="grow">
      <div class="font-sans font-medium text-lg">
        {{ core_labels.password_label }}
      </div>
      <q-input
        outlined
        type="password"
    ...
    <div class="flex items-center">
      <nuxt-link
        to="/"
        class="font-bold text-uppercase tracking-widest text-slate-600"
        >Volver</nuxt-link
      >
      <q-space></q-space>
      <q-btn
        unelevated
        :label="sign_up_labels.button_label"
        color="teal-14"
        type="submit"
        outline
        class="rounded-full text-capitalize font-sans"
      />
    </div>
  </q-form>
</template>
  ...
  switch (lvl) {
    ...
    case "min":
      return [
        ...
        sanitizationRules.rules(validatedData, "space_characters_max_length", {
          space_characters_max_length: 0,
        }),
      ];
      break;
    ...
    case "weak":
      return [
        ...
        sanitizationRules.rules(validatedData, "characters_min_length", {
          characters_min_length: 4,
        }),
      ];
      break;
...
const emit = defineEmits(["signUpSuccess", "signUpSuccessData"]);
